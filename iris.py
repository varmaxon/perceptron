...
INPUT_DIM = 4
OUT_DIM = 3
H_DIM = 10 # нейроны в скрытом слое
...
def softmax_batch(t):
    out = np.exp(t)
    return out / np.sum(out, axis=1, keepdims=True)  # матрицы (batch, num_classes)
...
for ep in range(NUM_EPOCHS):
    ...
    for i in range(len(dataset) // BATCH_SIZE):
        ...
        y_full = to_full_batch(y, OUT_DIM)  # one-hot encoding
        ...
        dE_dt1 = dE_dh1 * relu_derivate(t1)
        dE_dW1 = x.T @ dE_dt1
